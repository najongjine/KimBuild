	...
	<form:form commandName="boardVO" name="defaultFrm" id="defaultFrm" method="post">
			...
			<table class="tbl_row_type01"> 
				...
				<tbody>
					<c:choose>
						...
						<c:otherwise>
							<input type="hidden" name="title" id="title" class="text w100p"  required="required"  maxlength="50" value="reply" />
						</c:otherwise>
					</c:choose>
					
					<c:if test="${procType eq 'insert' or procType eq 'update'}">
						<tr>
							<th scope="row"><strong class="">비밀글</strong></th>
							<td colspan="3">
								<input type="checkbox" name="isPrivate" id="isPrivate" value="Y" 
								 <c:if test="${boardVO.isPrivate eq 'Y'}">checked</c:if> />
							</td>
						</tr>
					</c:if>
		...
		<div class="tbl_btns c">
			...
			<c:choose>
				...
				<c:otherwise><a href="#" class="btn btn_middle btn_gray" id="btn_list">취소</a></c:otherwise>
			</c:choose>
		</div>
	</form:form>
</div>