...
var showCmtUpdateForm=function(bcmtSeq){
	...
	let updateCmtFormHtml=`
		...
		<td>
			<a href="#" class="btn btn_middle btn_purple" onclick="submitUpdateCmt(\${bcmtSeq})" >수정</a> 
			<a href="#" class="btn btn_middle btn_gray" onclick="getCmtList()">취소</a>
		</td>
	`
						...
						<tr id="cmtTr_${result.bcmtSeq}" class="cursor">
							...
							<td>
								<c:if test="${result.rgstId eq loginFtSeq}">
									<a href="#" class="btn btn_middle btn_purple" onclick="showCmtUpdateForm(${result.bcmtSeq})" >수정</a> 
									<a href="#" class="btn btn_middle btn_gray" onclick="submitDeleteCmt(${result.bcmtSeq})">삭제</a>
								</c:if>
							</td>
						</tr>
					</c:forEach>
