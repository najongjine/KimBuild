package com.open.ma.mboard.mconsult;
 ...
 */
@Controller
public class MConsultController {
	...
	/** Program ID **/
	private final static String PROGRAM_ID = "Consult";

	/** folderPath **/
	private final static String folderPath = "/ma/mboard/mconsult/";
	...
	public String view(@ModelAttribute("searchVO") BReplyVO searchVO, Model model, HttpServletRequest request)
			throws Exception {

		/* 게시판 상세정보 */
		BReplyVO boardVO = new BReplyVO();
		boardVO = (BReplyVO) cmmnService.selectContents(searchVO, PROGRAM_ID);
		BReplyVO prevItem = (BReplyVO) cmmnService.selectContents(boardVO, PROGRAM_ID + ".selectPrevContents");
		BReplyVO nextItem = (BReplyVO) cmmnService.selectContents(boardVO, PROGRAM_ID + ".selectNextContents");
	...
	public String form(@ModelAttribute("searchVO") BReplyVO searchVO, Model model, @PathVariable String procType,
			HttpServletRequest request) throws Exception {
	...
	public String proc(@ModelAttribute("searchVO") BReplyVO searchVO, Model model, SessionStatus status,
		...
		if (procType != null) {
			...
			if (procType.equals("update")) {
				...
				model.addAttribute("message", "답변이 수정되었습니다.");
				model.addAttribute("pName", "seq");
				model.addAttribute("pValue", searchVO.getSeq());
				model.addAttribute("cmmnScript", "view.do");
				return "cmmn/execute";
			} else if (procType.equals("deleteReply")) {
				model.addAttribute("message", "답변이 삭제 되었습니다.");
				model.addAttribute("pName", "seq");
				model.addAttribute("pValue", searchVO.getSeq());
				model.addAttribute("cmmnScript", "view.do");
				return "cmmn/execute";
